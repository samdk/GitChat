...
class UpServer < EM::Connection
  ...
  def process_http_request
    ...
    response.content = '<center><h1>I\'m Running!</h1></center>'
    response.send_response
  end
end

AMQP.start(:host => "localhost") do
  puts "GitChat Server running"
  @mq = MQ.new
    ...
    def new_connection(message, ws)
      ...
      begin
        ...
        if old_conn = @old_connections[message['data']['uuid']]
          ...
          puts "Reconnecting on #{ws.signature}"
        else
